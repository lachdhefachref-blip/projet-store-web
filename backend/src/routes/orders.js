...
const createSchema = z.object({
  ...
  shipping: z.object({
    ...
    phone: z.string().min(5, "Phone must be at least 5 digits").max(40),
    address: z.string().min(3, "Address must be at least 3 chars").max(200),
  ...
  try {
    ...
    for (const it of parsed.data.items) {
      ...
      if (p.stock < it.quantity) {
        return res.status(400).json({ error: "out_of_stock", productId: it.productId });
      }
    ...
    res.status(500).json({ error: "create_order_failed", details: err.message });
  }
});
